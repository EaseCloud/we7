		...
		if (!empty($reply)) {	
			if (empty($_W['fans']['from_user'])) {//防止错误提交的数据
				...
				if (time() <= $reply['starttime'] || time() >= $reply['endtime'] || $reply['status'] == 0) {
					...
					$arr['p1'] = round(100 * $rate * $rate1 * $prizes['p1_num']/$p_num);
					$arr['p2'] = round(100 * $rate * $rate2 * $prizes['p2_num']/$p_num);
					$arr['p3'] = round(100 * $rate * $rate3 * $prizes['p3_num']/$p_num);
					$arr['p4'] = round(100 * $rate * $rate4 *$prizes['p4_num']/$p_num);
