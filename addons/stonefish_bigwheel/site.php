...
class stonefish_bigwheelModuleSite extends WeModuleSite {	
	...
	function Get_share($rid,$from_user,$title,$iid) {
		...
		if (!empty($iid)) {
		    $prize = pdo_fetch("select prizename FROM ".tablename("stonefish_bigwheel_prize")." where id = :id", array(':id' => $iid));
	...
	public function doMobileShareview() {
		...
		if(!empty($fromuser)) {
		    ...
		    if(!empty($fans)){
			    ...
			    $this->message_tips('抱歉，您的朋友没有参与本活动！请告之你的朋友，3秒后自动进入活动页！',url('entry//index',array('m'=>'stonefish_bigwheel','rid'=>$rid)),3);
		    }
			if($uid>=1){
				...
				$firend['headimgurl'] = '../addons/stonefish_bigwheel/template/images/avatar.jpg';
	...
	public function doMobileindex() {
		...
		if(empty($from_user)) {
			...
			if($reply['issubscribe']>=2){
				...
				if(empty($members)) {
				    $this->message_tips('请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','url'=>url('entry//index',array('m'=>'stonefish_bigwheel','rid'=>$rid)))),3);
				}
				if($members['status']==0) {
				    $this->message_tips('会员已被锁定，请联系管理员');
				}
				if($reply['issubscribe']==3 && empty($mobile['mobile'])) {
				    $this->message_tips('请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'mobile','url'=>url('entry//index',array('m'=>'stonefish_bigwheel','rid'=>$rid)))),3);
				}
				if($reply['issubscribe']==4 && empty($members['groupid'])) {
				    $this->message_tips('请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'groupid','url'=>url('entry//index',array('m'=>'stonefish_bigwheel','rid'=>$rid)))),3);
				}
				if($reply['issubscribe']==5 && empty($members['districtid'])) {
				    $this->message_tips('请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'districtid','url'=>url('entry//index',array('m'=>'stonefish_bigwheel','rid'=>$rid)))),3);
				}
			}
			//验证是否为会员 
			if($uid>=1){
		...
		if($reply['opportunity']==1){
			...
			if(empty($profile['mobile'])){
				$this->message_tips('还没有注册成为会员，无法进入刮奖', url('entry//member',array('m'=>'stonefish_member','url'=>url('entry//index',array('m'=>'stonefish_bigwheel','rid'=>$rid)))),3);
			}
		...
		if($isgift && ($fans['awardnum']<$reply['award_num'] || $reply['award_num']==0)){
            ...
            if($prizeid>0){
				...
				if($awardinfo['prizetype']!='physical' && $awardinfo['prizetype']!='virtual' && $awardinfo['prizetype']!='spaceprize' && $awardinfo['prizetype']!='againprize'){
				    ...
				    //减少库存
					//添加积分到粉丝数据库
			        mc_credit_update($_W['member']['uid'], $awardinfo['prizetype'], $awardinfo['prizevalue'], array($_W['member']['uid'], '刮中'.$awardinfo['prizevalue'].'个'.$credit_names));
			        //添加积分到粉丝数据库
				}
...
select * from ".tablename('stonefish_bigwheel_fansaward')." order by zhongjiang asc) as tt  where rid = :rid and uniacid = :uniacid and from_user = :from_user and zhongjiang>=1 GROUP BY prizeid order by `id` desc", array(':rid' => $rid,':uniacid' => $uniacid,':from_user' => $from_user));
	...
	public function doWebDeletefans() {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
			...
			//删除粉丝中奖记录			
			//删除粉丝中奖详细记录
			pdo_delete('stonefish_bigwheel_fansaward', array('from_user' => $fans['from_user']));
			//删除粉丝中奖详细记录
			//删除粉丝中奖详细记录
			pdo_delete('stonefish_bigwheel_sharedata', array('fromuser' => $fans['from_user']));
			//删除粉丝中奖详细记录
			//删除粉丝参与记录
