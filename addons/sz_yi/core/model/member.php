...
class Sz_DYi_Member
{
	...
	{
		global $_W;
		$_var_1 = intval($member_id);
		if (empty($_var_1)) {
			$_var_2 = pdo_fetch('select * from ' . tablename('sz_yi_member') . ' where  openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member_id));
		} else {
			$_var_2 = pdo_fetch('select * from ' . tablename('sz_yi_member') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $_var_1));
		}
		if (!empty($_var_2)) {
			$member_id = $_var_2['openid'];
			if (empty($_var_2['uid'])) {
				$_var_4 = m('user')->followed($member_id);
				if ($_var_4) {
					load()->model('mc');
					$_var_1 = mc_openid2uid($member_id);
					if (!empty($_var_1)) {
						$_var_2['uid'] = $_var_1;
			...
			$_var_6 = $this->getCredits($member_id);
